...
const EditUnits = () => {
	...
	useEffect(() => {
		// Fetch existing details based on user id
		const AllUnitDetails = async () => {
			try {
				setLoading(true);
	...
	const handleImageUpload = (e: ChangeEvent<HTMLInputElement>) => {
		...
		setUnitDetails((prevDetails) => ({
			...prevDetails,
			pictures: [...prevDetails.pictures, ...newPictures],
		}));
	};
	...
	const handleRemoveImage = (indexToRemove: number) => {
		...
		if (window.confirm("Are you sure you want to remove this image?")) {
			setUnitDetails((prevDetails) => ({
				...prevDetails,
				pictures: prevDetails.pictures.filter((_, index) => index !== indexToRemove),
			}));
		}
	};
	...
	const handleAddUnit = async (e: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			unitDetails.pictures.forEach((pic: File) => formData.append("pictures", pic));
			formData.append("name", unitDetails.name);
			formData.append("number", unitDetails.number);
			formData.append("type", unitDetails.type);
			formData.append("location", unitDetails.location);
			formData.append("status", unitDetails.status);
			formData.append("numberOfBedrooms", unitDetails.numberOfBedrooms);
			formData.append("price", unitDetails.price.replace(/,/g, ""));
			formData.append("description", unitDetails.description);
			...
			const Response = await axiosInstance.put(`/api/update-unit/${id}`, formData, config);
			...
			if (Response.status === 200) {
				...
				setLoading(false);
				toast.error("Failed to edit unit. Please try again.");
			}
		} catch (error) {
			...
			<Container>
				...
				<FormContainer>
					...
					<p
						...
						Edit available Unit to the list
					</p>
					{/* </Content> */}
					{/* <ToastContainer /> */}
					<form onSubmit={handleAddUnit}>
						...
						<DropZone onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={handleClick}>
							...
							{unitDetails.pictures.length > 0 ? (
								<div>
									{unitDetails.pictures.map((pic, index) => (
										<ImagePreviewContainer key={index}>
											{typeof pic === "string" ? (
												<FilePreview src={pic} alt={`Uploaded ${index + 1}`} />
											) : (
												<FilePreviewContainer>
													<p>Image Preview:</p>
													<FilePreview src={URL.createObjectURL(pic)} alt={`Uploaded ${index + 1}`} />
												</FilePreviewContainer>
											)}
						...
						<Button type="submit" disabled={loading}>
							...
							{loading ? "Adding..." : (<BtnTxt>Save & Continue</BtnTxt>)}
						</Button>
					</form>
				</FormContainer>
			</Container>
